...
* A featured section showcasing top-rated restaurants, complete with images, brief descriptions, and ratings. For this, you should fetch the top-rated restaurants provided in the following API endpoint: `https://valami.hu/api/v1/restaurants/top-rated`. Carefully review the returned JSON and display the content on the web page. If you cannot fetch the restaurants, there is some content in the `assets/content` folder that you can use to display them.
...
* Below the interactive search bar, there are the restaurant displayed. To access the restaurant data, you can use the following API endpoint: `https://valami.hu/api/v1/restaurants`. You should display the restaurants in random order. Initially, only the first 3 restaurants are displayed along with a 'Show more' button. Clicking this button reveals the other restaurants. If you cannot fetch the restaurants, there is some content in the `assets/content` folder that you can use to display them.
* Footer section with essential links, contact information, and social media integration. (Phase 1)

#### About Us Page (Phase 1):
...
* You are not allowed to use any other JS or CSS frameworks or libs (e.g Ember.js, Bootstrap, Tailwind etc.)
...
* You are allowed to use only the following Javascript frameworks: React, Angular, Vue. 
* You are not allowed to use any other JS or CSS frameworks or libs (e.g Ember.js, Bootstrap, Tailwind etc.) 
* To use of any additional npm module in the JS framework project is also forbidden. You can use npm modules only which installed by the default project generation process of the selected JS framework (create-react-app, ng new, vue create).
* However, you may use CSS prepocessors during development.
